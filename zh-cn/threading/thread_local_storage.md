#线程局部存储(Thread Local Storage,TLS)概述
...
&emsp;&emsp;举个Windows下的C语言例子来说, 如果我们想统计每个线程的内存使用情况, 没有TLS的话代码运行的时间大概是会在187毫秒左右(我电脑I7-8750H, 6核12线程):
```c
// File: main.c
#include <stdio.h>
#include <windows.h>
...
// 寻找线程信息的索引
unsigned int g_thread_info_index(unsigned int thread_id) {
  int index = -1;
  int cur_tid = 0;
  lock();
...
void thread_proc_inner(BOOL leak_it) {
  unsigned int index = g_thread_info_index(GetCurrentThreadId());
  void* ptr = malloc(1);
